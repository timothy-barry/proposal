...
sample_x_null <- function() {
  c(rnorm(n = n_null, mean = 0, sd = 1),
...
x_null_calibrate <- sample_x_null()
x_null <- sample_x_null()
x_alt <- rnorm(n = n_alt, mean = 3, sd = 1)
x_null_calibrate_trim <- trim_vector(x = x_null, trim_pnt = trim_pnt)

# x_null_trim <- trim_vector(x = x_null, trim_pnt = trim_pnt)
# x_alt_trim <- trim_vector(x = x_alt, trim_pnt = trim_pnt)

# plot the null density, both raw and truncated
my_col <- c("dodgerblue3", "firebrick3", "orange2")
p_calibrate_raw <- ggplot(data = data.frame(x = x_null_calibrate), mapping = aes(x = x)) + geom_histogram(bins = 32, alpha = 0.7, position = "identity", col = "black", fill = my_col[3]) + theme_bw() + xlab("") + ggtitle("Raw NC density") + theme(legend.position = c(0.8, 0.7), axis.text.y = element_blank(), axis.ticks.y = element_blank()) + ylab("")
p_calibrate_trim <- ggplot(data = data.frame(x = x_null_calibrate_trim), mapping = aes(x = x)) + geom_histogram(bins = 32, alpha = 0.7, position = "identity", col = "black", fill = my_col[3]) + theme_bw() + xlab("") + ggtitle("Truncated NC density") + theme(legend.position = c(0.8, 0.7), axis.text.y = element_blank(), axis.ticks.y = element_blank()) + ylab("")
s_calibrate_raw <- make_s_plot(x_null_calibrate) + ggtitle("Raw NC s-plot")
s_calibrate_trim <- make_s_plot(x_null_calibrate_trim) + ggtitle("Truncated NC s-plot")

p_calibrate_out <- cowplot::plot_grid(p_calibrate_raw, s_calibrate_raw, p_calibrate_trim, s_calibrate_trim, nrow = 2)
ggsave(filename = "/Users/timbarry/research_code/proposal/figs/bc_robustness_calibration.png", plot = p_calibrate_out, device = "png", scale = 1, width = 7, height = 5, dpi = 330)
